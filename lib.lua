...
--lib
local window = library:new({textsize = 13.5,font = Enum.Font.RobotoMono,name = "Ntd Service - [Project Slayer v1.5]",color = Color3.fromRGB(50, 168, 82)})

local Main_1_Page = window:page({name = "Main"})
local Main_1_left = Main_1_Page:section({name = "Auto Farm",side = "left",size = 1000})
local Main_1_right = Main_1_Page:section({name = "Misc",side = "left",size = 250})
--tab1
Main_1_left:toggle({name = "Auto Mob",def = false,callback = function(vu)
    maponeset.autofarm = vu
     
end})

Main_1_left:toggle({name = "Kill Aura",def = false,callback = function(vu)
    maponeset.killaura = vu
end})

Main_1_left:toggle({name = "Auto Lily",def = false,callback = function(vu)
    maponeset.autolily = vu
end})

Main_1_left:toggle({name = "God Mode [Kamado]",def = false,callback = function(vu)
    game:GetService("ReplicatedStorage").Remotes.heal_tang123asd:FireServer(true)
end})
--tab 2
  Main_1_right:button({name = "Auto Chest",callback = function()
     maponeset.autochest = true
end})

Main_1_right:button({name = "Auto Soul [Demon]",callback = function()
     maponeset.autosoul = true
end})

Main_1_right:button({name = "Asta Grimoire",callback = function()
   game:GetService("ReplicatedStorage").Remotes.PlayerCommunication:FireServer("relearnTransformation", "antiMagicGrimoire")
end})

Main_1_right:button({name = "Dark Grimoire",callback = function()
   game:GetService("ReplicatedStorage").Remotes.PlayerCommunication:FireServer("relearnTransformation", "darknessGrimoire")
end})
