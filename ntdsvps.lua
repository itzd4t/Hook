...
local Villages = {
    ...
    ["Zapiwara Cave"] = CFrame.new(-8.283 8707, 275.944641, -2414.72607, -0.795035779, -1.09233964-08, -0.606562555, 1.19429289e-09, 1, -1.95740775e-08, 0.606562555, -1.62865046e-08, -0.795035779),
...
local txt="Project Slayer"
local Window = CFAHub:CreateWindow("<font color=\"#4FC3F7\">CFA Hub</font>", txt, true)
local Tab2 = Window:CreatePage("Main")

local Section2 = Tab2:CreateSection("Main Farm")

local lf = Section2:CreateToggle("Auto Farm", {Toggled=Settings.Farm,Description = "Auto Farm lv must he select weapon"}, function(state)
    Settings.Farm = state
end)

getgenv().Method = "Sword"
Section2:CreateDropdown("Kill Aura Method", {
    List = Add{"Scythe","Sword","Combat"},
...
Section2:CreateDropdown("Farm Method", {
    List = Add{"Above","Below","Behind"},
    Default = getgenv().FarmMethod
}, function(v)
    getgenv().FarmMethod = v
end)

getgenv().tweenspeed = 10
Section2:CreateSlider("Tween Speed", {Min = 1, Max = 300, DefaultValue = 50}, function(value)
    getgenv().tweenspeed = value
 end)

getgenv().Distance = 5
Section2:CreateSlider("Farm Distance", {Min = 1, Max = 10, DefaultValue = 7}, function(value)
    getgenv().Distance = value
 end)

local Section2 = Tab2:CreateSection("Auto Skill")
Section2:CreateToggle("Use Skill", {Toggled=Settings.Farm,Description = "Select skill u want!"}, function(state)
    getgenv().AutoUseSkills = state
end)

Section2:CreateToggle("Z", {Toggled=getgenv().Z,Description = ""}, function(state)
    getgenv().Z = state
end)

Section2:CreateToggle("X", {Toggled=getgenv().X,Description = ""}, function(state)
    getgenv().X = state
end)

Section2:CreateToggle("C", {Toggled=getgenv().C,Description = ""}, function(state)
    getgenv().C = state
end)

Section2:CreateToggle("V", {Toggled=getgenv().V,Description = ""}, function(state)
    getgenv().V = state
end)

Section2:CreateToggle("B", {Toggled=getgenv().B,Description = ""}, function(state)
    getgenv().B = state
end)

Section2:CreateToggle("N", {Toggled=getgenv().N,Description = ""}, function(state)
    getgenv().N = state
end)

local Tab3 = Window:CreatePage("Misc")
Section2:CreateToggle("Infinite Breathing", {Toggled=getgenv().InfBreathing,Description = ""}, function(state)
    getgenv().InfBreathing = state
end)

Section2:CreateToggle("Infinite Saitama", {Toggled=getgenv().InfStamina,Description = ""}, function(state)
    getgenv().InfStamina = state
end)

Section2:CreateToggle("No Drown", {Toggled=getgenv().NoDrown,Description = ""}, function(state)
    getgenv().NoDrown = state
end)

Section2:CreateToggle("Auto Meditate", {Toggled=getgenv().AutoMed,Description = ""}, function(state)
    getgenv().AutoMed = state
end)

Section2:CreateToggle("Auto Split Boulder", {Toggled=getgenv().AutoSplit,Description = ""}, function(state)
    getgenv().AutoSplit = state
end)

Section2:CreateToggle("Thunder Mode - Human Only", {Toggled=nil,Description = ""}, function(state)
    game:GetService("ReplicatedStorage").Remotes.thundertang123:FireServer(true)
end)

Section2:CreateToggle("Ren Mode - Ren/Human Only", {Toggled=nil,Description = ""}, function(state)
    game:GetService("ReplicatedStorage").Remotes.heart_ablaze_mode_remote:FireServer(true)
end)

Section2:CreateToggle("God Mode - Kamado Only", {Toggled=nil,Description = ""}, function(state)
    game:GetService("ReplicatedStorage").Remotes.heal_tang123asd:FireServer(true)
end)

Section2:CreateToggle("Auto Blow", {Toggled=getgenv().AutoGourd,Description = ""}, function(state)
    getgenv().AutoGourd = state
end)
--Player
